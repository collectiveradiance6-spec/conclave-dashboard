...
const getBeaconToken = async () => {
  ...
  try {
    ...
    const res = await axios.post(
      "https://api.usebeacon.app/v4/login",
      params.toString(),
      { headers: { "Content-Type": "application/x-www-form-urlencoded" }, timeout: 10000 }
    );
    beaconToken = res.data.access_token || res.data.accessToken;
    const expiresIn = res.data.access_token_expires_in || res.data.expires_in || 3600;
    beaconTokenExpiry = Date.now() + (expiresIn * 1000) - 60000;
