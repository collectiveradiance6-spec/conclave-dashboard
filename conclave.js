...
const PAGE_CFG = {
  index:     {pal:['#FF4CD2','#A855F7','#5865F2','#00D4FF','#35ED7E','#FFB800','#FF0080'],bgHex:0x050508,pSz:.12,pOp:.52,lA:0x7B2FFF,lB:0x00D4FF,rise:false,spiral:false},
};

// Pixel sprites removed — using WebGL bg-engine instead
...
function initBG(pageType){ /* bg-engine v100 handles rendering */ }
...
function initCursor() {
  ...
  /* Inject cursor CSS — works regardless of external stylesheet */
  if(!document.getElementById('_cur_css')){
    const cs=document.createElement('style');cs.id='_cur_css';
    cs.textContent=[
      '#co{position:fixed;top:0;left:0;width:38px;height:38px;margin-top:-19px;margin-left:-19px;border-radius:50%;border:1.5px solid rgba(123,47,255,.7);pointer-events:none;z-index:99998;will-change:transform;transition:width .25s,height .25s,border-color .25s;}',
      '#ci{position:fixed;top:0;left:0;width:8px;height:8px;margin-top:-4px;margin-left:-4px;border-radius:50%;background:#fff;pointer-events:none;z-index:99999;will-change:transform;mix-blend-mode:difference;}',
      '.ct{position:fixed;top:0;left:0;border-radius:50%;pointer-events:none;will-change:transform;}',
      'body.ch #co{width:54px;height:54px;margin-top:-27px;margin-left:-27px;border-color:rgba(0,212,255,.9);background:rgba(0,212,255,.06);}',
      'body.cc #co{width:22px;height:22px;margin-top:-11px;margin-left:-11px;background:rgba(255,255,255,.18);}',
    ].join('');
...
function initReveal() {
  ...
  const obs = new IntersectionObserver(entries => {
    entries.forEach(e => {
      ...
      setTimeout(() => {
        e.target.classList.add('in'); e.target.classList.add('vis');
...
function fetchGoal(cb) {
  fetch(API+'/api/donation-goal').then(r=>r.json()).then(cb).catch(()=>fetch(API+'/donation-goal').then(r=>r.json()).then(cb))
...
function fetchServers(cb) {
  ...
  fetch(url)
    ...
    .then(d => cb(Array.isArray(d) ? {servers:d} : d))
    .catch((err) => {
      console.error('[fetchServers] failed:', err, 'URL:', url);
...
async function aegisChat(iid,cid) {
  ...
  ch.innerHTML+=`<div id="at" style="padding:.55rem .9rem;border-radius:8px;margin-bottom:.5rem;background:rgba(0,212,255,.07);border:1px solid rgba(0,212,255,.15);font-size:.8rem;font-family:var(--fm);color:#00D4FF;opacity:.5;">AEGIS is thinking...</div>`;
  ch.scrollTop=ch.scrollHeight;
  try {
    const r=await fetch(API+'/api/aegis/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg})});
    const d=await r.json();
    document.getElementById('at')?.remove();
    ch.innerHTML+=`<div style="padding:.55rem .9rem;border-radius:8px;margin-bottom:.5rem;background:rgba(0,212,255,.07);border:1px solid rgba(0,212,255,.15);font-size:.8rem;font-family:var(--fm);color:#00D4FF"><div style="font-size:.6rem;color:rgba(0,212,255,.4);margin-bottom:.15rem;">AEGIS</div>${d.reply||'Realm intelligence active.'}</div>`;
    ch.scrollTop=ch.scrollHeight;
  } catch(e) {
    document.getElementById('at')?.remove();
    const r=AR[Math.floor(Math.random()*AR.length)];
    ch.innerHTML+=`<div style="padding:.55rem .9rem;border-radius:8px;margin-bottom:.5rem;background:rgba(0,212,255,.07);border:1px solid rgba(0,212,255,.15);font-size:.8rem;font-family:var(--fm);color:#00D4FF"><div style="font-size:.6rem;color:rgba(0,212,255,.4);margin-bottom:.15rem;">AEGIS</div>${r}</div>`;
    ch.scrollTop=ch.scrollHeight;
  }
}
function _aegisChat_legacy(iid,cid) {
  const inp=document.getElementById(iid),ch=document.getElementById(cid);
  if(!inp||!ch)return;const msg=inp.value.trim();if(!msg)return;
  inp.value='';
  ch.innerHTML+=`<div style="padding:.55rem .9rem;border-radius:8px;margin-bottom:.5rem;background:rgba(123,47,255,.12);border:1px solid rgba(123,47,255,.22);font-size:.8rem;font-family:var(--fm)"><div style="font-size:.6rem;color:rgba(255,255,255,.28);margin-bottom:.15rem;">YOU</div>${msg}</div>`;
...
function init(pageType) {
  enterAnim();
  if(window.CLBG){try{CLBG.init(pageType);}catch(e){console.warn('[TC-BG]',e.message);}}
  // CLBG background engine active
  initCursor();
  initSpotlight();
  initMagnetic();
  initNav();
