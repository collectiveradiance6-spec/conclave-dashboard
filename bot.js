...
// Grace period — don't watch until 90s after boot
...
setInterval(async () => {
  ...
  /* Discord.js v14 WebSocket status codes:
     0 = READY          ✅ healthy
     1 = CONNECTING     🔄 transient — Discord.js is working, don't interfere
     2 = RECONNECTING   🔄 transient
     3 = IDLE           🔄 transient
     4 = NEARLY         🔄 transient — about to become READY
     5 = DISCONNECTED   ❌ actual problem
     6 = WAITING_FOR_GUILDS  🔄 transient
     7 = IDENTIFYING    🔄 transient
     8 = RESUMING       🔄 transient
  */
  const HEALTHY    = [0, 1, 2, 3, 4, 6, 7, 8]; // all non-5 states are Discord handling itself
  const BROKEN     = wsStatus === 5;

  if (HEALTHY.includes(wsStatus)) {
    if (wsStatus === 0) { watchdogFails = 0; lastReady = Date.now(); }
    return; // Discord.js is handling reconnect — don't interfere
  }

  if (!BROKEN) return; // unknown state — leave it alone

  watchdogFails++;
  const downSec = Math.floor((Date.now() - lastReady) / 1000);
  console.warn(`⚠️  Watchdog: ws=${wsStatus} down=${downSec}s fails=${watchdogFails}`);

  // Only act after 5 consecutive DISCONNECTED checks (2.5 min fully dead)
  if (watchdogFails >= 5) {
    STATUS.reconnects++;
    console.error(`❌ Bot fully disconnected — restarting process (attempt #${STATUS.reconnects})...`);
    STATUS.ready = false;
