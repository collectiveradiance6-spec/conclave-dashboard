...
// ─── SERVER STATUS VIA BATTLEMETRICS HTTP API ─────────
// BattleMetrics indexes all public ARK servers automatically
// No API key needed for basic queries - uses HTTP not UDP
...
const fetchServerStatuses = async () => {
  ...
  const results = {};
  console.log("🔍 Querying servers via BattleMetrics...");

  await Promise.all(SERVERS.map(async (srv) => {
    try {
      // Search by IP:port using BattleMetrics public API
      const url = `https://api.battlemetrics.com/servers?filter[search]=${srv.ip}:${srv.port}&filter[game]=arksa&fields[server]=name,players,maxPlayers,status,ip,port&page[size]=1`;
      ...
      // Find exact match by port
      const match = list.find(s =>
        String(s.attributes.port) === String(srv.port) ||
        String(s.attributes.ip) === String(srv.ip)
      ) || list[0];

      if (match) {
        const isOnline = match.attributes.status === "online";
        results[srv.id] = {
          ...srv,
        ...
        console.log(`${isOnline ? "✅" : "❌"} ${srv.name}: ${results[srv.id].status} (${results[srv.id].players}/${srv.maxPlayers})`);
      } else {
        console.log(`⚠️ ${srv.name}: not found on BattleMetrics`);
        results[srv.id] = { ...srv, status: "unknown", players: 0, maxPlayers: srv.maxPlayers };
      }
    } catch (e) {
      console.error(`❌ BM query failed for ${srv.name}:`, e.message);
      results[srv.id] = { ...srv, status: "unknown", players: 0, maxPlayers: srv.maxPlayers };
    }
  }));
