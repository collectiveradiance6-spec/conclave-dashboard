...
const getBeaconToken = async () => {
  ...
  try {
    ...
    const res = await axios.post(
      "https://api.usebeacon.app/v4/oauth2/token",
      params.toString(),
      { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
    );
...
const fetchServerStatuses = async () => {
  ...
  try {
    ...
    // Fetch all Sentinel services (servers)
    const res = await axios.get(
      "https://api.usebeacon.app/v4/sentinel/services",
      {
    ...
    console.log(`📡 Beacon returned ${services.length} services`);
    if (services.length > 0) console.log("Sample service keys:", Object.keys(services[0]));

    // Match by beaconId (serviceId) first, then fall back to IP:port
    SERVERS.forEach(srv => {
      const service = services.find(s =>
        s.serviceId === srv.beaconId ||
        String(s.serviceId) === String(srv.beaconId) ||
        (String(s.address || s.ip || "").includes(srv.ip) && (s.port === srv.port || String(s.port) === String(srv.port)))
      );
      if (service) {
        const isOnline = ["connected","online","running"].includes(String(service.status || "").toLowerCase()) || service.online === true;
        results[srv.id] = {
          ...srv,
          status: isOnline ? "online" : "offline",
          players: service.onlinePlayers ?? service.playerCount ?? service.players ?? 0,
          maxPlayers: srv.maxPlayers,
          version: service.version || "v84.16",
        };
      } else {
