...
const fetchServerStatuses = async () => {
  ...
  await Promise.all(SERVERS.map(async (srv) => {
    try {
      ...
      const url = `https://api.battlemetrics.com/servers?filter[search]=${srv.ip}:${srv.port}&filter[game]=ark&fields[server]=name,players,maxPlayers,status,ip,port&page[size]=1`;
