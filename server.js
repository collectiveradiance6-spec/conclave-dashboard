...
// ─── DIRECT ARK SERVER QUERY ──────────────────────────
// Uses Source Engine Query Protocol (ARK supports this natively)
// Query port = game port + 1 for Nitrado servers

const dgram = require("dgram");

const queryServer = (ip, gamePort) => {
  return new Promise((resolve) => {
    ...
    client.on("message", (msg) => {
      ...
      client.close();
      try {
        if (msg.length < 6) return resolve({ online: false, players: 0 });
        // Parse A2S_INFO response
        let offset = 4;
        const header = msg.readUInt8(offset++);
        if (header !== 0x49) return resolve({ online: true, players: 0 });
        // Skip protocol, name, map, folder, game, appid
        while (offset < msg.length && msg.readUInt8(offset) !== 0) offset++;
        offset++; // null term
        while (offset < msg.length && msg.readUInt8(offset) !== 0) offset++;
        offset++;
        while (offset < msg.length && msg.readUInt8(offset) !== 0) offset++;
        ...
        resolve({ online: true, players: players || 0 });
      } catch {
        resolve({ online: true, players: 0 });
      }
    });
    ...
    client.send(query, 0, query.length, queryPort, ip, (err) => {
      if (err) {
        clearTimeout(timeout);
        try { client.close(); } catch {}
        resolve({ online: false, players: 0 });
...
const fetchServerStatuses = async () => {
  ...
  const results = {};
  console.log("🔍 Querying all servers directly...");

  await Promise.all(SERVERS.map(async (srv) => {
    try {
      const result = await queryServer(srv.ip, srv.port);
      results[srv.id] = {
        ...srv,
      ...
      console.log(`${result.online ? "✅" : "❌"} ${srv.name}: ${result.online ? "online" : "offline"} (${result.players}/${srv.maxPlayers})`);
    } catch (e) {
      console.error(`❌ Query failed for ${srv.name}:`, e.message);
